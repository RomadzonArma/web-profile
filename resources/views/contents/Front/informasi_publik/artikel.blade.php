...
@section('content')
    <div class="col-md-9 col-12">
        ...
        <div class="result-artikel">
            @foreach ($artikel as $item)
                <div class="entry mb-5">
                    <div class="grid-inner row no-gutters p-0">
                        <div class="entry-image col-md-4 mb-md-0">
                            <a href="{{ route('artikel.detail', ['slug' => $item->slug]) }}">
                                <img src="{{ asset('/storage/uploads/gambar-artikel/' . $item->gambar) }}" alt="thumbnail_artikel">
                            </a>
                        </div>
...
{{--
            <ul class="pagination pagination-circle justify-content-center">
                ...
                @foreach (range(1, $artikel->lastPage()) as $page)
                    <li class="page-item{{ $page == $artikel->currentPage() ? ' active' : '' }}">
                        <a class="page-link" href="{{ $artikel->url($page) }}">{{ $page }}</a>
                    </li>
                @endforeach

                @if ($artikel->hasMorePages())
                    <li class="page-item"><a class="page-link" href="{{ $artikel->nextPageUrl() }}" aria-label="Next"><span
            ...
            <ul class="pagination pagination-circle justify-content-center">
                @if ($artikel->onFirstPage())
                    <li class="page-item disabled"><span class="page-link" aria-hidden="true">«</span></li>
                @else
                    <li class="page-item"><a class="page-link" href="{{ $artikel->previousPageUrl() }}" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
                @endif

                @php
                    $limit = 5;
                    $halfLimit = floor($limit / 2);
                    $start = $artikel->currentPage() - $halfLimit;
                    $end = $artikel->currentPage() + $halfLimit;
                    ...
                    if ($end > $artikel->lastPage()) {
                        $start -= $end - $artikel->lastPage();
                        $end = $artikel->lastPage();
                    }
                    if ($start < 1) {
                        $start = 1;
                    }
                ...
                @for ($page = $start; $page <= min($end, $artikel->lastPage()); $page++)
                    <li class="page-item{{ $page == $artikel->currentPage() ? ' active' : '' }}">
                        <a class="page-link" href="{{ $artikel->url($page) }}">{{ $page }}</a>
                    </li>
                @endfor
                ...
                @if ($artikel->hasMorePages())
                    <li class="page-item"><a class="page-link" href="{{ $artikel->nextPageUrl() }}" aria-label="Next"><span aria-hidden="true">»</span></a></li>
                @else
                    <li class="page-item disabled"><span class="page-link" aria-hidden="true">»</span></li>
                @endif
            </ul>
