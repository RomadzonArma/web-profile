        ...
        <div class="result-panduan">
            ...
            <ul class="pagination pagination-circle justify-content-center">
                @if ($panduan->onFirstPage())
                    <li class="page-item disabled"><span class="page-link" aria-hidden="true">«</span></li>
                @else
                    <li class="page-item"><a class="page-link" href="{{ $panduan->previousPageUrl() }}" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
                @endif

                @php
                    $limit = 5;
                    $halfLimit = floor($limit / 2);
                    $start = $panduan->currentPage() - $halfLimit;
                    $end = $panduan->currentPage() + $halfLimit;

                    if ($start < 1) {
                        $end += 1 - $start;
                        $start = 1;
                    }
                    if ($end > $panduan->lastPage()) {
                        $start -= $end - $panduan->lastPage();
                        $end = $panduan->lastPage();
                    }
                    if ($start < 1) {
                        $start = 1;
                    }
                ...
                @for ($page = $start; $page <= min($end, $panduan->lastPage()); $page++)
                    <li class="page-item{{ $page == $panduan->currentPage() ? ' active' : '' }}">
                        <a class="page-link" href="{{ $panduan->url($page) }}">{{ $page }}</a>
                    </li>
                @endfor
                ...
                @if ($panduan->hasMorePages())
                    <li class="page-item"><a class="page-link" href="{{ $panduan->nextPageUrl() }}" aria-label="Next"><span aria-hidden="true">»</span></a></li>
                @else
                    <li class="page-item disabled"><span class="page-link" aria-hidden="true">»</span></li>
                @endif
            </ul>
