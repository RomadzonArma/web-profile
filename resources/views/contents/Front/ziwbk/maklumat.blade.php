...
@section('content')
    <div class="col-md-9 col-12">
        <form class="row mb-4" method="get" action="{{ url('/maklumat/list') }}">
        ...
        <div class="result-maklumat">
            @foreach ($maklumat as $item)
                <div class="entry mb-5">
                    <div class="grid-inner row no-gutters p-0">
                        {{-- <div class="entry-image col-md-4 mb-md-0">
                            <a href="{{ route('maklumat.detail', ['id' => $item->id]) }}">
                                <img src="{{ asset($item->gambar) }}" alt="img">
                            </a>
                        </div> --}}
                        <div class="col-md-8 pl-md-4">
                            <div class="entry-title title-md">
                                <h3 class="mb-1"><a href="{{ route('maklumat.detail', ['id' => $item->id]) }}">{{ $item->judul_maklumat }}</a>
                                </h3>
                            </div>
                            <div class="entry-meta mb-2 mt-0">
                                <ul>
                                    ...
                                    <li><a href="#"><i class="icon-line-folder"></i> Maklumat</a></li>
                                    {{-- <li>
                                        <a href="#">
                            ...
                            <a class="more-link" href="{{ route('maklumat.detail', ['id' => $item->id]) }}">Baca Lebih Lanjut</a> --}}
                        </div>
                    </div>
                </div>
            @endforeach
            ...
            <ul class="pagination pagination-circle justify-content-center">
                @if ($maklumat->onFirstPage())
                    <li class="page-item disabled"><span class="page-link" aria-hidden="true">«</span></li>
                @else
                    <li class="page-item"><a class="page-link" href="{{ $maklumat->previousPageUrl() }}"
                            aria-label="Previous"><span aria-hidden="true">«</span></a></li>
                @endif

                @foreach (range(1, $maklumat->lastPage()) as $page)
                    <li class="page-item{{ $page == $maklumat->currentPage() ? ' active' : '' }}">
                        <a class="page-link" href="{{ $maklumat->url($page) }}">{{ $page }}</a>
                    </li>
                @endforeach

                @if ($maklumat->hasMorePages())
                    <li class="page-item"><a class="page-link" href="{{ $maklumat->nextPageUrl() }}" aria-label="Next"><span
