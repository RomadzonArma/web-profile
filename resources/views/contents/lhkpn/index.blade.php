        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header" style="background-color: #365984;">
                    <h5 class="modal-title text-white mt-0" id="modal-customLabel">Form LHKPN</h5>
...
<script>
    ...
    function tambah() { 
        $.ajax({
            ...
            beforeSend: () => {
                ...
                $('#modal-custom').find('#modal-customLabel').html(`Form Tambah LHKPN`)
    ...
    $('#table-data').on('click', '.btn-update', function () {
        ...
        $.ajax({
            ...
            beforeSend: () => {
                $('#modal-custom').find('.modal-body').html(`<div class="d-flex align-items-center justify-content-center" style="min-height: 300px;"><img src="{{ asset('assets/images/loading.gif') }}" style="width: auto;"></div>`)
                $('#modal-custom').find('#modal-customLabel').html(`Form Ubah LHKPN`)
                $('#modal-custom').modal('show')
            }
        }).then(res => {
            $('#modal-custom').find('.modal-body').html(res)
    ...
    function lihat_dokumen(id) {
        $.ajax({
            type: "POST",
            url: "{{ route('lhkpn.lihat_dokumen') }}",
            data: {id:id},
            dataType: "HTML",
            beforeSend: () => {
                $('#modal-custom').find('.modal-body').html(`<div class="d-flex align-items-center justify-content-center" style="min-height: 300px;"><img src="{{ asset('assets/images/loading.gif') }}" style="width: auto;"></div>`)
                $('#modal-custom').find('#modal-customLabel').html(`Lihat Dokumen LHKPN`)
                $('#modal-custom').modal('show')
            }
        }).then(res => {
            $('#modal-custom').find('.modal-body').html(res)
    ...
    function upload_dokumen(ctx) {
        ...
        $.ajax({
            type: "POST",
            url: "{{ route('lhkpn.upload_dokumen') }}",
            data: formData,
            dataType: "JSON",
            processData: false,
            contentType: false,
