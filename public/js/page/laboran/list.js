...
$(() => {
    ...
    $("#table-data").on("click", ".btn-delete", function () {
        ...
        Swal.fire({
            title: "Anda yakin?",
            html: `Anda akan menghapus laboran"<b>${judul}</b>"!`,
            ...
            if (result.isConfirmed) {
                $.post(BASE_URL + "laboran/delete", {
                    id,
                    _method: "DELETE",
                })
                    .done((res) => {
                        showSuccessToastr("sukses", "laboran berhasil dihapus");
                        table.ajax.reload();
                    })
                    .fail((res) => {
        ...
        $.post(BASE_URL + "laboran/switch", {
            ...
            .done((res) => {
                showSuccessToastr(
                    "sukses",
                    value == "1"
                        ? "laboran berhasil diaktifkan"
                        : "laboran berhasil dinonaktifkan"
                );
                table.ajax.reload();
            })
            .fail((res) => {
    ...
    $("#form-laboran-update").on("submit", function (e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: (res) => {
                ...
                clearErrorMessage();
                table.ajax.reload();
                $("#modal-laboran-update").modal("hide");
                showSuccessToastr("sukses", "laboran berhasil diubah");
            },
        ...
        if (data) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $("#updatelaboranPreview").empty(); // Kosongkan pratinjau sebelumnya
                    if (
                        response &&
                        response.status &&
                        ...
                        $("#updatelaboranPreview").text("No images found");
                    }
                    console.log("Response:", response);
                    $("#modal-laboran-update").modal("show");
                },
                error: function (xhr, status, error) {
    ...
    $("#form-laboran").on("submit", function (e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: (res) => {
                $("#modal-laboran")
                    .find(".modal-dialog")
                    .LoadingOverlay("hide", true);
                clearErrorMessage("#modal-laboran"); // Ubah clearErrorMessage() menjadi clearErrorMessage("#modal-laboran")
                // $(this)[0].reset(); // Hapus ini
                table.ajax.reload();
                $("#imagePreview").html("");
                $("#konten").val("");
                $("#modal-laboran").modal("hide");
                showSuccessToastr(
                    "sukses",
                    "laboran baik berhasil ditambahkan"
                );
            },
    ...
    $(".btn-tambah").on("click", function () {
        $("#form-laboran")[0].reset();
        clearErrorMessage();
        $("#modal-laboran").modal("show");
        ...
        $("#modal-laboran").find(".row_pdf").hide();
        $("#modal-laboran").find(".row_foto").hide();
    });
    ...
    $("#table-data").on("click", ".btn-detail", function (e) {
        ...
        $.ajax({
            url: BASE_URL + "laboran/get-images/" + laboranId,
            type: "GET",
            dataType: "json",
            success: function (response) {
                if (response.status) {
                ...
                render: (data, type, row) => {
                    if (data) {
                        const buttonDetail = $("<a>", {
                            ...
                            "data-placement": "top",
                            "data-toggle": "tooltip",
                            href: BASE_URL + "laboran/get-images/" + row.id,
                            role: "button",
