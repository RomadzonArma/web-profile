    ...
    $('#table-data').on('click', '.btn-delete', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: BASE_URL + 'webinar/delete/' + id,
                    type: 'DELETE',
        ...
        // },
        {
            ...
            render: (data, type, row) => {
                const button_edit = $('<a>', {
                    ...
                    href: BASE_URL + 'webinar/edit/' + row.id,
                    'data-id': data,
                    title: 'Update Data',
                    'data-placement': 'top',
                    'data-toggle': 'tooltip'
...
$('#table-data').on('change', '.switch-active', function () {
    ...
    $.post(BASE_URL + 'webinar/switch', {
        id,
        value,
        _method: 'PATCH'
    }).done((res) => {
        showSuccessToastr('sukses', value == '1' ? 'Webinar berhasil di publish' : 'Webinar berhasil di unpublish');
        table.ajax.reload();
    }).fail((res) => {
