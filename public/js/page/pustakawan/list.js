...
$(() => {
    ...
    $("#table-data").on("click", ".btn-delete", function () {
        ...
        Swal.fire({
            title: "Anda yakin?",
            html: `Anda akan menghapus pustakawan baik "<b>${judul}</b>"!`,
            ...
            if (result.isConfirmed) {
                $.post(BASE_URL + "pustakawan/delete", {
                    id,
                    _method: "DELETE",
                })
                    .done((res) => {
    ...
    $("#table-data").on("change", ".switch-active", function () {
        ...
        $.post(BASE_URL + "pustakawan/switch", {
            id,
            value,
            _method: "PATCH",
        })
            .done((res) => {
                showSuccessToastr(
                    "sukses",
                    value == "1"
                        ? "pustakawan berhasil diaktifkan"
                        : "pustakawan berhasil dinonaktifkan"
                );
                table.ajax.reload();
            })
            .fail((res) => {
    ...
    $("#form-pustakawan-update").on("submit", function (e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: (res) => {
                ...
                clearErrorMessage();
                table.ajax.reload();
                $("#modal-pustakawan-update").modal("hide");
                showSuccessToastr("sukses", "pustakawan berhasil diubah");
            },
    ...
    $("#modal-pustakawan-update").on("hidden.bs.modal", function () {
        // Mengatur ulang form
        $("#form-pustakawan-update")[0].reset();
    ...
    //         var pdfExists = data["file_pdf"] && data["file_pdf"].endsWith(".pdf");
    //         // Check if image exists in database
    //         var imageExists = data["gambar"];

    //         // Hide both sections by default
    //         $(".row_pdf, .row_foto").hide();

    //         // Show PDF section if PDF exists, otherwise show image section if image exists
    //         if (pdfExists) {
    //             $(".row_pdf").show();
    ...
    //             $("#updatepustakawanPreview").html(
    //                 '<img src="' + data["gambar"] + '" alt="foto" style="width: 200px;">'
    //             );
    //         }

    //         // Populate other input fields normally
    //         $.each(data, (key, value) => {
    //             if (key !== "file_pdf" && key !== "gambar") {
    //                 $("#update-" + key).val(value);
    //             }
    //         });

    //         // Show the update modal
    //         $("#modal-pustakawan-update").modal("show");
    //     } else {
        ...
        if (data) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $("#updatepustakawanPreview").empty(); // Kosongkan pratinjau sebelumnya
                    if (response && response.status && response.status === true && response.images && response.images.length > 0) {
                        ...
                        response.images.forEach(function(image) {
                            $("#updatepustakawanPreview").append(
                        ...
                        $("#updatepustakawanPreview").text("No images found");
                    }
    ...
    $("#form-pustakawan").on("submit", function (e) {
        ...
        $.ajax({
            ...
            success: (res) => {
                ...
                clearErrorMessage();
                table.ajax.reload();
                $("#modal-pustakawan").modal("hide");
                showSuccessToastr(
                    "sukses",
                    "pustakawan baik berhasil ditambahkan"
                );
            },
    ...
    $(".btn-tambah").on("click", function () {
        ...
        $("#modal-pustakawan").find(".row_pdf").hide();
        $("#modal-pustakawan").find(".row_foto").hide();
    });
    ...
    $("#table-data").on("click", ".btn-detail", function (e) {
        ...
        $.ajax({
            url: BASE_URL + "pustakawan/get-images/" + laboranId,
            type: "GET",
            dataType: "json",
            success: function (response) {
                if (response.status) {
                ...
                render: (data, type, row) => {
                    if (data) {
                        const buttonDetail = $("<a>", {
                            ...
                            "data-placement": "top",
                            "data-toggle": "tooltip",
                            href: BASE_URL + "pustakawan/get-images/" + row.id,
                            role: "button",
