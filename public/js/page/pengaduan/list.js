...
$(() => {
    ...
    $('body').on('click', '.btn-tambah', function (e) {
        ...
        $('#tambah-list-pengaduan').modal('show');

    })

    $('#tambah-list-pengaduan').on('shown.bs.modal', function (e) {
    ...
    $('#table-data').on('click', '.btn-delete', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: BASE_URL + 'pengaduan/delete/' + id,
                    type: 'DELETE',
                    success: function (response) {
                        Swal.fire({
                            icon: "success",
...
$('#table-data').on('change', '.switch-active', function () {
    ...
    $.post(BASE_URL + 'pengaduan/switch', {
        id,
        value,
        _method: 'PATCH'
    }).done((res) => {
        showSuccessToastr('sukses', value == '1' ? 'pengaduan berhasil di publish' : 'pengaduan berhasil di unpublish');
        table.ajax.reload();
    }).fail((res) => {
        let {
            status,
...
$('body').on('click', '.btn-update', function (e) {
    ...
    $.ajax({
        url: BASE_URL + 'pengaduan/edit/' + id,
        type: 'GET',
        success: function (response) {
            $('#edit-list-pengaduan').modal('show');
            $('#id_kategori_edit').val(response.result.id_kategori);
            $('#link_pengaduan_edit').val(response.result.link_pengaduan);
            ...
            $("#form-update").submit(function (e) {
                ...
                $.ajax({
                    url: BASE_URL + 'pengaduan/update/' + id,
                    type: "POST",
                    data: formData,
                    dataType: "JSON",
                    processData: false,
