...
$(() => {
    ...
    $('#table-data').on('click', '.btn-delete', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: BASE_URL + 'berita_zi_wbk/delete/' + id,
                    type: 'DELETE',
    ...
    table = $('#table-data').DataTable({
        ...
        processing: true,
        ajax: {
            url: BASE_URL + 'berita_zi_wbk/data',
            type: 'get',
            dataType: 'json'
        },
        order: [
...
$('#table-data').on('change', '.switch-active', function () {
    ...
    $.post(BASE_URL + 'berita_zi_wbk/switch', {
        id,
        value,
        _method: 'PATCH'
    }).done((res) => {
        showSuccessToastr('sukses', value == '1' ? 'Berita ZI/WBK berhasil di publish' : 'Berita ZI/WBK berhasil di unpublish');
        table.ajax.reload();
    }).fail((res) => {
